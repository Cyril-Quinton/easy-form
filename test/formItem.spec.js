      ...
      children: [
        {
          descriptor: {
            ...
            validateFunction: (userValue, answers) => { return userValue === "toto" && answers.get("top_s1_q2") > 2 }
          }
        },
        {
          descriptor: {
...
describe("FormItem tests", () => {
  ...
  expect(formIds[1]).to.equal("top_s1");
  expect(formIds[2]).to.equal("top_s1_q1");
  expect(formIds[3]).to.equal("top_s1_q2");
  expect(formIds[4]).to.equal("top_s2");
  expect(formIds[5]).to.equal("top_s2_q1");
});
...
it('validates and saves user inputs correctly', () => {
  ...
  const formArray = form.toArray();
  const qtoto = formArray.find(item => item.id === "top_s1_q1");
  const qNumber = formArray.find(item => item.id === "top_s1_q2");

  qNumber.setValue(-1);
  expect(qNumber.validated).to.equal(false);
  expect(answersMap.get("top_s1_q2")).to.equal(undefined);

  qtoto.setValue("toto");
  expect(qtoto.validated).to.equal(false);
  expect(answersMap.get("top_s1_q1")).to.equal(undefined);

  qNumber.setValue(3);
  expect(qNumber.validated).to.equal(true);
  expect(answersMap.get("top_s1_q2")).to.equal(3);

  // The qNumber question has changed, qToot should be validated automatically. 
  expect(qtoto.validated).to.equal(true);
  expect(answersMap.get("top_s1_q1")).to.equal("toto");

  qNumber.setValue(1);
  expect(qNumber.validated).to.equal(true);
  expect(answersMap.get("top_s1_q2")).to.equal(1);
...
describe("Navigator tests", () => {
  ...
  const expected = ['top', 'top_s1', 'top_s1_q1'];
...
it("the currentNav should be updated while changing the current item to another leaf", () => {
  ...
  const newCurrentItem = form.toArray().find(item => item.id === 'top_s2_q3');
  nav.currentItem = newCurrentItem;
  const expected = ['top', 'top_s2', 'top_s2_q3'];
...
it("the currentNav should be updated while changing the current item to another inner node", () => {
  ...
  const newCurrentItem = form.toArray().find(item => item.id === 'top_s2');
  nav.currentItem = newCurrentItem;
  const expected = ['top', 'top_s2', 'top_s2_q2'];
  const actual = nav.currentNav.map(item => item.id);
  compareArrays(expected, actual);
...
it("the navigator should return the next leaf correctly", () => {
  ...
  expect(nextLeaf.id).to.equal("top_s1_q2");

  nav.goToNext();
  nextLeaf = nav.nextLeaf();
  expect(nextLeaf.id).to.equal("top_s1_q3");

  nav.goToNext();
  nextLeaf = nav.nextLeaf();
  expect(nextLeaf.id).to.equal("top_s2_q2");

  nav.goToNext();
  nextLeaf = nav.nextLeaf();
  expect(nextLeaf.id).to.equal("top_s2_q3");
